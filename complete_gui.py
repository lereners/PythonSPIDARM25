...
def audio_display(file_path):
    ...
    t = np.linspace(0, length, len(data), endpoint=False)
    time_display.config(text=f'Time: {length}')
    ...
    if channel_num == 1:
        ...
        res_display.config(text=f'Resonant Frequency: {res_freq}')
    ...
    if channel_num > 1:
        ...
        for x in range(channel_num):
            ...
            calc_rt60(spectrum, freq, t, "r", label_value=label_title)
...
def plot_spec():
    ...
    if len(data.shape) == 1:
        ...
        spec_data = data[:,0]
    spec_subplot.specgram(spec_data, Fs=sample_rate, NFFT=1024, cmap="autumn_r")
    spec_subplot.set_title("Spectrogram")
...
def calc_rt60(spectrum, freq, t, color, label_value):
    ...
    rt60_subplot.plot(t, data_in_db, color=color, linewidth=1, alpha=0.7, label=label_value)
